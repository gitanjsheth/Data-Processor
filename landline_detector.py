    ...
    for pattern_idx, pattern in enumerate(_PATTERNS):
        for match in pattern.finditer(text):
            ...
            if pattern_idx == 0:  # landline/landline pattern
                ...
                # Extract digits from both local parts upfront
                local1_digits = re.sub(r'[^\d]', '', local1)
                local2_digits = re.sub(r'[^\d]', '', local2)
                
                # Process first landline if it has sufficient digits
                if (len(std1) + len(local1_digits) >= 10) and std1 in STD_CODES:
                ...
                # Process second landline if it has sufficient digits
                if (len(std2) + len(local2_digits) >= 10) and std2 in STD_CODES:
                    ...
                    if second_zero_pos != -1:
                        ...
                        for i in range(second_start, len(text)):
                            if text[i].isdigit():
                                digits_removed += 1
                                if digits_removed == 10:  # Total digits for complete landline (STD + local)
                                    precise_end = i + 1
                                    break
                ...
                # Extract digits from local part upfront
                digits_only = re.sub(r'[^\d]', '', local_raw)
                
                # Check if STD code is valid and has sufficient total digits
                if std_part in STD_CODES and (len(std_part) + len(digits_only) >= 10):
